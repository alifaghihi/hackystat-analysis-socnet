...
public class GraphManager {
   ...
   * @return
   */
  public SocialMediaNode getNode(SocialMediaNode startNode, ArrayList<String> relationshipPath,
      ArrayList<String> relationshipPathDirections, ArrayList<String> nodeNamesOnPath) {
    ...
    try {
      for (int i = 0; i < relationshipPath.size(); i++) {
        ...
        node = nodeBuilder.getNode(startNode, BetweenNodesRelationshipType.getEnum(relationshipPath
  ...
  public SocialMediaNode getNode(String nodeType, String nodeName) throws NodeNotFoundException {
    ...
    Node underNode;
    try {
      underNode = nodeBuilder.getNode(IsARelationshipType.getEnum(nodeType),
  ...
  public void addNode(String nodeType, String name) {
    ...
    try {
      if (nodeBuilder
          .getNode(IsARelationshipType.getEnum(nodeType), SocialMediaNode.NAME_KEY, name) != null) {
        return; // create the node
      }
      SocialMediaNode node = nodeBuilder.createNode(IsARelationshipType.getEnum(nodeType));
  ...
  public void relateNodes(String node1Type, String node1Name, String node2Type, String node2Name,
      String relationship) throws NodeNotFoundException {
    Transaction tx = neo.beginTx();

    try {
      nodeBuilder.relateNodes(IsARelationshipType.getEnum(node1Type), SocialMediaNode.NAME_KEY,
          node1Name, IsARelationshipType.getEnum(node2Type), SocialMediaNode.NAME_KEY, node2Name,
  ...
  public List<XMLNode> getNodes(XMLNode connectedTo, String relationshipType, String direction)
    ...
    try {
      // get the nodes of the appropriate type from the database
      nodes = nodeBuilder.getNodes(connectedTo.getName(), IsARelationshipType.getEnum(connectedTo
          .getType()), BetweenNodesRelationshipType.getEnum(relationshipType),
  ...
  public List<XMLRelationship> getRelationships(XMLNode connectedTo)
    ...
    try {
      ...
      rels = nodeBuilder.getNode(IsARelationshipType.getEnum(connectedTo.getName()),
          SocialMediaNode.NAME_KEY, connectedTo.getName()).getRelationships();

      if (rels == null) {
        throw new RelationshipNotFoundException(connectedTo.getName(), connectedTo.getType());
      }
      for (Relationship rel : rels) {
        // wrap each node in a SocialMediaNode wrapper
        xmlRels.add(convertToXMLRelationship(new SocialMediaRelationship(rel)));
  ...
  public List<XMLRelationship> getRelationships(XMLNode connectedTo, String relationshipType,
      String direction) throws RelationshipNotFoundException, InvalidArgumentException {
    // start a new transaction
    Transaction tx = neo.beginTx();
    ...
    try {
      ...
      rels = nodeBuilder.getNode(IsARelationshipType.getEnum(connectedTo.getName()),
          SocialMediaNode.NAME_KEY, connectedTo.getName()).getRelationships(
          IsARelationshipType.getEnum(relationshipType), getDirection(direction));
  ...
  public void addRelationship(XMLRelationship rel) throws NodeNotFoundException {
    ...
    try {
      ...
      Relationship r = nodeBuilder.relateNodes(IsARelationshipType.getEnum(startNode.getType()),
          SocialMediaNode.NAME_KEY, startNode.getName(), IsARelationshipType.getEnum(endNode
              .getType()), SocialMediaNode.NAME_KEY, endNode.getName(),
      ...
      if (startNode.getType().equals(IsARelationshipType.IS_HACKYSTAT_ACCOUNT)
          && r.getType().equals(BetweenNodesRelationshipType.CONTRIBUTES_TO)
          && endNode.getType().equals(IsARelationshipType.IS_PROJECT) && streams.size() == 9) {
  ...
  public XMLRelationship getRelationship(String type, XMLNode startNode, XMLNode endNode)
    ...
    XMLRelationship xmlRel = null;
    try {
      Relationship rel = nodeBuilder.getRelationship(BetweenNodesRelationshipType.getEnum(type),
          IsARelationshipType.getEnum(startNode.getType()), SocialMediaNode.NAME_KEY, startNode
              .getName(), IsARelationshipType.getEnum(endNode.getType()), SocialMediaNode.NAME_KEY,
          endNode.getName());

      if (rel == null) {
        throw new RelationshipNotFoundException(type, startNode.getName(), startNode.getType(),
  ...
  public static boolean areEqual(XMLNode n1, XMLNode n2) {
    ...
    if (n1.getName().equals(n2.getName()) && n1.getType().equals(n2.getType())) {
      areEqual = true;
    }
    return areEqual;
  }
  ...
  public void updateRelationship(XMLRelationship r) throws NodeNotFoundException {
    ...
    try {
      ...
      Relationship rel = nodeBuilder.getRelationship(BetweenNodesRelationshipType.getEnum(r
          .getType()), IsARelationshipType.getEnum(startNode.getType()), SocialMediaNode.NAME_KEY,
          startNode.getName(), IsARelationshipType.getEnum(endNode.getType()),
