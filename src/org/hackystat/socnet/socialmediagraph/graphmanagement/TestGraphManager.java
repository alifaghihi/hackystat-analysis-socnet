...
public class TestGraphManager {
    ...
    @Test
    public void testGraphAddAndRetrieveNode() throws DatatypeConfigurationException, NodeNotFoundException
    {
        assertTrue("Testing putting a node in and retrieving from the graph",
                isSuccessfulAddAndRetrieveNode());
    }
    
    public static boolean isSuccessfulAddAndRetrieveNode() throws DatatypeConfigurationException, NodeNotFoundException 
    {
        boolean wasSuccessful = false;
        
        String directory = "/home/cody/.hackystat/socnet/testdb/graph";
        ...
        if(smNode.hasName() && smNode.hasType())
            if(smNode.getName().equals(name) && smNode.getType().equals(type))
                return true;
    ...
    @Test
    public void testGraphAddAndRetrieveRelationship() throws DatatypeConfigurationException, NodeNotFoundException, RelationshipNotFoundException
    {
        assertTrue("Testing putting two nodes and a relationship in and retrieving them from the graph",
                isSuccessfulAddAndRetrieveRelationship());
    }
    
    public static boolean isSuccessfulAddAndRetrieveRelationship() throws DatatypeConfigurationException, NodeNotFoundException, RelationshipNotFoundException 
    {
        boolean wasSuccessful = false;
        ...
        gm.relateNodes(node1type, node1name, node2type, node2name, relationshipType);
        ...
        XMLRelationship xmlRel = gm.getRelationship(relationshipType, xmlNode1, xmlNode2);
        
        if(xmlRel.getXMLNode().get(0).getName().equals(node1name) 
                && xmlRel.getXMLNode().get(1).getName().equals(node2name)
                && xmlRel.getXMLNode().get(0).getType().equals(node1type) 
                && xmlRel.getXMLNode().get(1).getType().equals(node2type)
                && xmlRel.getType().equals(relationshipType))
                return true;
    ...
    @Test
    public void testGraphRetrieveAllNodes() throws DatatypeConfigurationException, NodeNotFoundException
    {
        assertTrue("testGraphRetrieveAllNodes", isSuccessfulRetrieveAllNodes());
    }
    
    public static boolean isSuccessfulRetrieveAllNodes() throws DatatypeConfigurationException, NodeNotFoundException
    {
        boolean wasSuccessful = false;
        ...
        String node2name = "HackystatSocNet";
        String node2type = IsARelationshipType.IS_TWITTER_ACCOUNT.name();
        String relationshipType1to2 = BetweenNodesRelationshipType.IS_FOLLOWING.name();
        ...
        gm.relateNodes(node1type, node1name, node2type, node2name, relationshipType1to2);
    ...
    @Test
    public void testGraphRetrieveNodesOfSpecifiedType() throws DatatypeConfigurationException, NodeNotFoundException
    {
        assertTrue("testGraphRetrieveAllNodes", isSuccessfulRetrieveAllNodes());
    }
    
    public static boolean isSuccessfulRetrieveNodesOfSpecifiedType() throws DatatypeConfigurationException, NodeNotFoundException
    {
        boolean wasSuccessful = false;
