...
public class HackystatPoller
{
    ...
    public static final String CYCLOMATIC_COMPLEXITY_DEFAULTS =
            "AverageComplexityPerMethod,10,JavaNCSS";
    ...
    {
        BUILD_DEFAULTS, CHURN_DEFAULTS, CODE_ISSUE_DEFAULTS,
        COMMIT_DEFAULTS, COVERAGE_DEFAULTS, CYCLOMATIC_COMPLEXITY_DEFAULTS,
        DEV_TIME_DEFAULTS, ISSUE_DEFAULTS,
        UNIT_TEST_DEFAULTS
    };
    public static final String[] CHART_NAMES = {"Build", "Churn", "CodeIssue", "Commit", 
    "Coverage", "CyclomaticComplexity", "DevTime", "Issue", "UnitTest"};

    public HackystatPoller(SensorBaseClient sbc, SocNetClient snc,
            TelemetryClient tc, String userEmail)
    ...
    public void addProject(String projectName, Date start, Date end) throws
            SensorBaseClientException
    {
        ...
        projectStarts.put(projectName, Tstamp.makeTimestamp(startCal.
                getTimeInMillis()));
        ...
        endCal.setTime(end);
        projectEnds.put(projectName, Tstamp.makeTimestamp(
                endCal.getTimeInMillis()));

        if (projectEnds.get(projectName).equals(sensorBaseClient.getProject(
                email, projectName).getEndTime()))
        {
    ...
    public org.hackystat.socnet.server.resource.socialmediagraph.jaxb.TelemetryStream getProjectStream(
            String chartName, String project)
            throws TelemetryClientException, SensorBaseClientException
    {
        ...
        else if (now.compare(projectEnds.get(project)) ==
        ...
        org.hackystat.telemetry.service.resource.chart.jaxb.TelemetryChartData tcd 
                = telemetryClient.getChart(chartName, email, project, "Day",
               dateLastSent.get(project), end);

        List<org.hackystat.telemetry.service.resource.chart.jaxb.TelemetryStream> stream =
                tcd.getTelemetryStream();
    ...
    {
        org.hackystat.socnet.server.resource.socialmediagraph.jaxb.TelemetryStream socnetStream =
                new org.hackystat.socnet.server.resource.socialmediagraph.jaxb.TelemetryStream();

        if (hackystatStream.isSetTelemetryPoint())
        {
            List<org.hackystat.socnet.server.resource.socialmediagraph.jaxb.TelemetryPoint> socnetPoints =
                    socnetStream.getTelemetryPoint();
            List<org.hackystat.telemetry.service.resource.chart.jaxb.TelemetryPoint> hackystatPoints =
                    hackystatStream.getTelemetryPoint();
        ...
        {
            socnetStream.setYAxis(
                    convertYAxisObjects(hackystatStream.getYAxis()));
        }
    ...
    {
        org.hackystat.socnet.server.resource.socialmediagraph.jaxb.TelemetryPoint socnetPoint =
                new org.hackystat.socnet.server.resource.socialmediagraph.jaxb.TelemetryPoint();
    ...
    {
        org.hackystat.socnet.server.resource.socialmediagraph.jaxb.YAxis socnetYAxis =
                new org.hackystat.socnet.server.resource.socialmediagraph.jaxb.YAxis();
    ...
    private XMLRelationship buildRelationship(String project)
            throws TelemetryClientException, SensorBaseClientException
    {

        XMLRelationship rel = new XMLRelationship();
        List<org.hackystat.socnet.server.resource.socialmediagraph.jaxb.TelemetryStream> relStreams =
                rel.getTelemetryStream();
    ...
    public void sendRelationships() throws JAXBException,
            ParserConfigurationException, TransformerConfigurationException,
            TransformerException, TelemetryClientException,
            SensorBaseClientException
    {
    ...
    public void updateRelationship(String projectName) throws 
            TelemetryClientException, JAXBException, 
            ParserConfigurationException, TransformerConfigurationException, 
    ...
    public void update() throws SensorBaseClientException, Exception
    {
        ...
        {
            dateLastSent.put(projectName, socNetClient.getDateLastUpdated(
                    email, IsARelationshipType.IS_HACKYSTAT_ACCOUNT.name(), 
                    projectName, IsARelationshipType.IS_PROJECT.name(), 
                    BetweenNodesRelationshipType.CONTRIBUTES_TO.name(),
                    dateLastSent.get(projectName).toXMLFormat())); //not sure about the toXMLFormat part
            
            if(!isUserSetEndDate.get(projectName))
            {               
                projectEnds.put(projectName, sensorBaseClient.getProject(email, 
                        projectName).getEndTime());
            }
