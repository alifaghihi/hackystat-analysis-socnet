...
public class Server extends Application {
  ...
  /**
   * Creates a new instance of a SensorBase HTTP server, listening on the supplied port. SensorBase
   * properties are initialized from the User's sensorbase.properties file.
   * 
  ...
  /**
   * Creates a new instance of a SensorBase HTTP server suitable for unit testing. SensorBase
   * properties are initialized from the User's sensorbase.properties file, then set to their
   * "testing" versions.
   * 
   * @return The Server instance created.
   * @throws Exception If problems occur starting up this server.
  ...
  public static Server newInstance(ServerProperties serverProperties) throws Exception {
    ...
    server.hostName = "http://" + server.serverProperties.get(HOSTNAME_KEY) + ":"
    ...
    // Now create all of the Resource Managers and store them in the
    // Context.
  ...
  public Restlet createRoot() {
    // First, create a Router that will have a Guard placed in front of it
    // so that this Router's
    ...
    authRouter
        .attach(
            "/relationships/{relationshiptype}/{startnodetype}/{startnodename}/{endnodetype}/{endnodename}",
            RelationshipResource.class);
    authRouter
        .attach(
            "/relationships/{relationshiptype}/{startnodetype}/{startnodename}/{endnodetype}/{endnodename}/{lastupdated}",
            RelationshipResource.class);
    ...
    // Now create our "top-level" router which will allow the registration
    // URI to proceed without
    // authentication, but all other URI patterns will go to the guarded
    // Router.
  ...
  public static String getVersion() {
    String version = Package.getPackage("org.hackystat.socnet.server").getImplementationVersion();
